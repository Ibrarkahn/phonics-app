...
async function trackEvent(name, params = {}) {
  try {
    console.log("trackEvent called:", name, params);

    // Native Capacitor app
    if (window.Capacitor && window.Capacitor.isNativePlatform && window.Capacitor.isNativePlatform()) {
      const plugin = window.Capacitor?.Plugins?.FirebaseAnalytics;

      console.log("Capacitor native:", true);
      console.log("FirebaseAnalytics plugin:", plugin);

      if (plugin && plugin.logEvent) {
        await plugin.logEvent({
          name,
          params
        });
        console.log("✅ Native analytics sent:", name, params);
        return;
      }
    }

    // Web fallback
    if (window.fbAnalytics?.analytics && window.fbAnalytics?.logEvent) {
      window.fbAnalytics.logEvent(window.fbAnalytics.analytics, name, params);
      console.log("✅ Web analytics sent:", name, params);
      return;
    }
...
function updateHomeLocks(){
  ...
  for (const key of WEEK_ORDER){
    ...
    if (completed.has(key)){
      ...
      setWeekButtonLockVisual(btn, false);
      ...
      setWeekButtonLockVisual(btn, true);
    }
  }
...
function playSoundFor(key, source = 'tap'){
  ...
  if (source === 'tap') {
    ...
    const weekKey = ACTIVE_WEEK_KEY || PRACTICE_KEY;
    if (weekKey) params.week_key = weekKey;

    console.log("LETTER PLAYED:", key);
...
function playBlend(word){
  ...
  const step = () => {
    ...
    if (rawKey === 'oo') {
      const OO_SHORT_WORDS = new Set(['good', 'book', 'look', 'cook', 'hook', 'foot', 'wood']);
...
function showCelebration({ title, practised, extra, weekKey }){
  ...
  if (ACTIVE_WEEK_KEY) celebrateParams.week_key = ACTIVE_WEEK_KEY;
  trackEvent("celebrate_screen_viewed", celebrateParams);
...
function setupWeek({
  ...
  let lIdx = 0;
  let wIdx = 0;
  let lastLetterTouchTime = 0;
  let lastBlendTouchTime = 0;

  function renderLetterLocal(){
    const key = letters[lIdx] ?? '';
    const label = getDisplayLabel(key);

    if (bigLetter){
      if (key.length === 2 && key[0] === key[1]){
        bigLetter.textContent = `${label[0]}\u2009${label[1]}`;
      } else {
  ...
  if (letterArea){
    letterArea.addEventListener('click', (e)=> {
      if (Date.now() - lastLetterTouchTime < 700) {
        e.preventDefault();
        return;
      }
  ...
  if (blendArea){
    blendArea.addEventListener('click', (e)=> {
      if (Date.now() - lastBlendTouchTime < 700) {
        e.preventDefault();
        return;
      }
    ...
    blendArea.addEventListener('touchstart', e => {
      sx = e.changedTouches[0].clientX;
    }, { passive:true });
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  safeOn('#btn-practise','click', () => {
    trackEvent('practice_mode_used', { mode: 'alphabet' });
    PRACTICE_KEY = null;
    PRACTICE_SEEN_LAST = false;
  ...
  document.querySelectorAll('[data-open-term]').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      let termName = '';
      if (target === 'term-autumn') termName = 'autumn';
      if (target === 'term-spring') termName = 'spring';
      if (target === 'term-summer') termName = 'summer';

      const rawText = (btn.textContent || '').trim().toLowerCase();
      const source = (rawText === 'open →' || rawText === 'open→' || rawText === 'open') ? 'button' : 'link';

      trackEvent('term_opened', {
        term: termName,
        source: source
      });
